...
#
# the segment information came from an email from reMarkable support.
#
# Eric S Fraga
import sys
import struct
...
def rm2svg(input_file, output_name, coloured_annotations=False,
    ...
    output.write('<svg xmlns="http://www.w3.org/2000/svg" height="{}pt" width="{}pt" viewBox="0 0 {} {}">'.format(y_width, x_width, x_width, y_width)) # BEGIN Notebook
    output.write('''
    ...
    for layer in range(nlayers):
        ...
        for stroke in range(nstrokes):
            if is_v3:
                ...
                fmt = '<IIIffI'
                pen, colour, i_unk, width, i_unk2, nsegments = struct.unpack_from(fmt, data, offset); offset += struct.calcsize(fmt)

            # print('pen={} colour={} i_unk={} width={} i_unk2={} nsegments={}'.format(pen, colour, i_unk, width, i_unk2, nsegments))
            opacity = 1
            last_x = -1.; last_y = -1.
            #if i_unk != 0: # No theory on that one
                #print('Unexpected value at offset {}'.format(offset - 12))
            if (pen == 2 or pen == 4 or pen == 15 or pen == 17): # Pen / Fineliner
                ...
                colour = 2
            elif (pen == 1 or pen == 7 or pen == 13 or pen == 14):     # Pencil-Sharp
                width = 16 * width - 27
                opacity = 0.9
